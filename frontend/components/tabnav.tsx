...
export default function Tabs({ busynessData }: BusynessData) {
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(busynessData) != JSON.stringify([[[1]]])) {
      setUpdated(true);
    }
  }, [busynessData]);
  return (
              ...
              {(() => {
                try {
                  ...
                  if (busynessData && Array.isArray(busynessData)) {
                    ...
                    return (
                      <div className="flex flex-row justify-center mt-10 items-center w-full">
                        <AreaGraph receivedData={processData(tabData)} empty={!updated} />
                      </div>
                    );
                } catch (error) {
                  console.error(error);
